...
async function callCatalogApi(path, body) {
  ...
  const response = await fetch(`${CATALOG_HOST}${path}`, {
    method: "POST",
    // Next.js caches server-side fetch() calls by default. Without this,
    // three "retry" attempts for the same search keyword could all silently
    // return the same cached response instead of asking Amazon again —
    // which would explain why retries weren't changing anything.
    cache: "no-store",
    headers: {
