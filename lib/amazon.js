...
// several plausible field names rather than betting on just one. We also
// reject implausible results (a "discount" implying the item was ever
// priced at more than 5x its current price is almost certainly bad data,
// not a real deal) rather than trusting an undocumented field blindly.
...
export function rankBestProducts(products, limit = 6, minDiscount = 0) {
  ...
  function score(p) {
    ...
    return rating * (1 + reviewWeight) + discountOf(p) * 6; // discount weighted more heavily
  }
  return [...products]
