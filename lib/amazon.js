...
//
// SAFETY: whatever shape we end up trusting, we reject implausible results
// (a "discount" implying the item was ever priced at more than 5x its
// current price, or a computed list price that isn't actually higher than
// price) rather than trusting an undocumented field blindly.
function extractListPrice(price, listing) {
  ...
  if (savings) {
    ...
    if (savingsAmount) {
      candidate = price + savingsAmount;
    } else {
      const savingsPercent = savings.percentage ?? savings.percent ?? null;
      if (savingsPercent && savingsPercent > 0 && savingsPercent < 100) {
        candidate = price / (1 - savingsPercent / 100);
      }
    }
  }
  ...
  if (!candidate) {
    ...
    if (deal) {
      ...
      if (percentOff && percentOff > 0 && percentOff < 100) {
        candidate = price / (1 - percentOff / 100);
      } else {
        candidate =
  ...
  if (!candidate || candidate <= price || candidate > price * 5) return null;
...
function mapItem(item) {
  ...
  const additionalImages = (item.images?.variants || [])
    .map((v) => v.large?.url)
    .filter(Boolean)
    .slice(0, 6);
...
/** Search Amazon by keyword, returns up to 10 matching products. `page`
 *  lets callers page through results (1-10) instead of always getting the
 *  same top 10 for a given keyword. */
export async function searchProductsByKeyword(keyword, page = 1) {
 ...
 * than Amazon's raw order. minDiscount (0-1) filters out anything below
 * that discount fraction — e.g. 0.5 keeps only 50%+ off items.
 */
export function rankBestProducts(products, limit = 6, minDiscount = 0) {
  function discountOf(p) {
    return p.list_price && p.list_price > p.price ? (p.list_price - p.price) / p.list_price : 0;
  }
  function score(p) {
    if (!p.price) return -1;
    ...
    return rating * (1 + reviewWeight) + discountOf(p) * 6;
  }
  return [...products]
    .filter((p) => discountOf(p) >= minDiscount)
