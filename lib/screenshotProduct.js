...
export async function captureAndUploadProductScreenshot(productUrl) {
  ...
  try {
    ...
    // Doesn't add any wait time — just rejects the capture if it matches
    // Amazon's bot-check interstitial page, so that page never gets
    // uploaded as the post image. Falls back to the stored product photo
    // when this triggers (see callers of this function).
