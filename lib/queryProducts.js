...
export async function queryProducts(options = {}) {
  ...
  if (searchQuery) {
    ...
    if (creatorsChoiceOnly) results = results.filter((p) => p.is_creators_choice);
  ...
  if (countOnly) {
    ...
    if (creatorsChoiceOnly) countQuery = countQuery.eq("is_creators_choice", true);
  ...
  if (creatorsChoiceOnly) query = query.eq("is_creators_choice", true);
