...
export { buildSingleProductCaption };
...
export async function postToSelectedPlatforms(product, { caption, platforms } = {}) {
  // Backs the "Post to social media" modal — the person picks which
  // platform(s) to post to and can edit the caption first, so this posts
  // to exactly what was requested rather than always doing FB+WhatsApp.
  // Instagram is included here (unlike the old quick two-button flow)
  // because this now runs from an explicit "Post" click the person
  // waits on, not a fire-and-forget card button — the caller's route
  // gives this a longer maxDuration to cover Instagram's polling wait.
  if (!product?.image_url) {
    return { skipped: true, reason: "Product has no image, so it wasn't posted." };
  }

  const finalCaption = caption || buildSingleProductCaption(product, false);
  const imageUrl = await getPostImage(product);

  const wantFacebook = !platforms || platforms.includes("facebook");
  const wantInstagram = !platforms || platforms.includes("instagram");
  const wantWhatsapp = !platforms || platforms.includes("whatsapp");

  const [fb, ig, wa] = await Promise.allSettled([
    wantFacebook ? postToFacebookPage(imageUrl, finalCaption) : Promise.resolve({ skipped: true }),
    wantInstagram ? postToInstagram(imageUrl, finalCaption) : Promise.resolve({ skipped: true }),
    wantWhatsapp ? postToWhatsAppChannel(imageUrl, finalCaption) : Promise.resolve({ skipped: true }),
  ]);

  const settle = (result, wanted) =>
    !wanted
      ? { skipped: true }
  ...
  return {
    facebook: settle(fb, wantFacebook),
    instagram: settle(ig, wantInstagram),
    whatsapp: settle(wa, wantWhatsapp),
  };
}
