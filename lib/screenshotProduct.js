...
export async function captureAndUploadProductScreenshot(productUrl) {
  ...
  try {
    const params = new URLSearchParams({
      ...
      // product page — waiting for the title element to appear (rather
      // than a fixed delay alone) skips capturing that interstitial. But
      // the title can appear in the DOM slightly before the rest of the
      // page (image, price box) has actually finished rendering, which is
      // why `delay` below is set generously — it's extra settle time
      // applied after that wait, not instead of it.  If the title never
      // appears at all within the timeout (still stuck on the
      // interstitial), error_on_selector_not_found makes the whole
      // request fail instead of silently capturing the wrong page — the
      // caller already falls back to the product's own stored image
